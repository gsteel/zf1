...
class Zend_Date extends Zend_Date_DateObject
{
     ...
     *                                                ,depending on $part. If null the actual time is
     *                                                set
    ...
    {
        if (is_object($date) and !($date instanceof Zend_TimeSync_Protocol) 
            and !($date instanceof Zend_Date)
        ) {
        ...
        if (($date !== null) and !is_array($date) and !($date instanceof Zend_TimeSync_Protocol) 
            and !($date instanceof Zend_Date) and !defined($date) and Zend_Locale::isLocale($date, true, false)
        ) {
            $locale = $date;
            $date   = null;
            $part   = null;
    ...
    public function toString($format = null, $type = null, $locale = null)
    {
        ...
        if (($type !== null) and ($type != 'php') and ($type != 'iso') 
            and Zend_Locale::isLocale($type, null, false)
        ) {
            $locale = $type;
            $type = null;
        }
    ...
    private function _calculate($calc, $date, $part, $locale)
    {
        ...
        if (is_array($date) === true) {
            if (empty($part) === false) {
                ...
                return $this->_assign(
                    $calc, $this->mktime($hours, $minutes, $seconds, $months, $days, $years, true),
        ...
        case self::DAY:
            if (is_numeric($date)) {
                return $this->_assign(
                    $calc, $this->mktime(0, 0, 0, 1, 1 + intval($date), 1970, true),
        ...
        case self::WEEKDAY_SHORT:
            ...
            if ($cnt < 7) {
                return $this->_assign(
                    $calc, $this->mktime(0, 0, 0, 1, 1 + $found, 1970, true),
        ...
        case self::WEEKDAY_DIGIT:
            ...
            if (is_numeric($date) and (intval($date) >= 0) and (intval($date) < 7)) {
                return $this->_assign(
                    $calc, $this->mktime(0, 0, 0, 1, 1 + $date, 1970, true),
                    $this->mktime(0, 0, 0, 1, 1 + $weekday, 1970, true), $hour
                );
            }
        ...
        case self::DAY_OF_YEAR:
            if (is_numeric($date)) {
                ...
                return $this->_assign(
                    $calc, $this->mktime(0, 0, 0, 1, $date, $year, true),
        ...
        case self::WEEK:
            if (is_numeric($date)) {
                ...
                return $this->_assign(
                    $calc, parent::mktime(0, 0, 0, 1, 1 + ($date * 7), 1970, true),
        ...
        case self::MONTH_NAME:
            ...
            if ($cnt < 12) {
                ...
                return $this->_assign(
                    $calc, $this->mktime(0, 0, 0, $date,  $day + $fixday, $year, true),
                    $this->mktime(0, 0, 0, $month, $day, $year, true), $hour
                );
            }
        ...
        case self::YEAR_8601:
            if (is_numeric($date)) {
                ...
                return $this->_assign(
                    $calc, $this->mktime(0, 0, 0, $month, $day, intval($date), true),
        ...
        case self::YEAR_SHORT:
            if (is_numeric($date)) {
                ...
                return $this->_assign(
                    $calc, $this->mktime(0, 0, 0, $month, $day, $date, true),
                    $this->mktime(0, 0, 0, $month, $day, $year, true), false
                );
            }
        ...
        case self::SWATCH:
            if (is_numeric($date)) {
                ...
                return $this->_assign(
                    $calc, $this->mktime($hours, $minutes, $seconds, 1, 1, 1970, true),
        ...
        case self::HOUR_SHORT_AM:
            if (is_numeric($date)) {
                return $this->_assign(
                    $calc, $this->mktime(intval($date), 0, 0, 1, 1, 1970, true),
        ...
        case self::MINUTE:
            if (is_numeric($date)) {
                return $this->_assign(
                    $calc, $this->mktime(0, intval($date), 0, 1, 1, 1970, true),
        ...
        case self::SECOND:
            if (is_numeric($date)) {
                return $this->_assign(
                    $calc, $this->mktime(0, 0, intval($date), 1, 1, 1970, true),
        ...
        case self::ISO_8601:
            ...
            return $this->_assign(
                $calc, $this->mktime($timematch[1], $timematch[2], $timematch[3], 1 + $datematch[2], 1 + $datematch[3], 1970 + $datematch[1], false),
        ...
        case self::RFC_2822:
            ...
            return $this->_assign(
                $calc, $this->mktime($match[4], $match[5], $match[6], 1 + $months, 1 + $match[1], 1970 + $match[3], false),
                $this->mktime($hour,     $minute,   $second,   1 + $month,  1 + $day,      1970 + $year,     false), false
            );
                break;
        ...
        case self::DATES:
            try {
                ...
                return $this->_assign(
                    $calc, $this->mktime(0, 0, 0, 1 + $parsed['month'], 1 + $parsed['day'], 1970 + $parsed['year'], true),
        ...
        case self::TIMES:
            try {
                ...
                return $this->_assign(
                    $calc, $this->mktime($parsed['hour'], $parsed['minute'], $parsed['second'], $month, $day, $year, true),
        ...
        case self::DATETIME:
            try {
                ...
                return $this->_assign(
                    $calc, $this->mktime($parsed['hour'], $parsed['minute'], $parsed['second'], 1 + $parsed['month'], 1 + $parsed['day'], 1970 + $parsed['year'], true),
        ...
        case self::RFC_3339:
            ...
            return $this->_assign(
                $calc, $this->mktime($match[4], $match[5], $match[6], 1 + $match[2], 1 + $match[3], 1970 + $match[1], true),
        ...
        case self::COOKIE:
            ...
            return $this->_assign(
                $calc, $this->mktime($match[4], $match[5], $match[6], 1 + $months, 1 + $match[1], 1970 + $match[3], true),
                $this->mktime($hour,     $minute,   $second,   1 + $month,  1 + $day,      1970 + $year,     true), false
            );
                break;
    ...
    private function _time($calc, $time, $format, $locale)
    {
        ...
        if ($time instanceof Zend_Date) {
            ...
            if (is_array($time)) {
                if ((isset($time['hour']) === true) or (isset($time['minute']) === true) 
                    or (isset($time['second']) === true)
                ) {
                    $parsed = $time;
                } else {
                    // require_once 'Zend/Date/Exception.php';
    ...
    private function _date($calc, $date, $format, $locale)
    {
        ...
        if ($date instanceof Zend_Date) {
            ...
            if (is_array($date)) {
                if ((isset($date['year']) === true) or (isset($date['month']) === true) 
                    or (isset($date['day']) === true)
                ) {
                    $parsed = $date;
                } else {
                    // require_once 'Zend/Date/Exception.php';
