...
class Zend_Config_Yaml extends Zend_Config
{
    ...
    protected static function _parseValue($value)
    {
        ...
        if ('"' == $value['0']) {
            if ('"' == $value[(is_array($value) || $value instanceof \Countable ? count($value) : 0) -1]) {
                $value = substr($value, 1, -1);
            }
        } elseif ('\'' == $value['0'] && '\'' == $value[(is_array($value) || $value instanceof \Countable ? count($value) : 0) -1]) {
